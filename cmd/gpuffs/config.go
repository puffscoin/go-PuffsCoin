...
//
// PUFFScoin is an independent iteration of the EVM. 
// PUFFScoin is maintained by The Leafy Cauldron Apothecary, LLC
// http://puffscoin.leafycauldronapothecary.com
//
...
import (
	...
	"github.com/puffscoin/go-puffscoin/graphql"
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	if ctx.GlobalIsSet(utils.GraphQLEnabledFlag.Name) {
		if err := graphql.RegisterGraphQLService(stack, cfg.Node.GraphQLEndpoint(), cfg.Node.GraphQLCors, cfg.Node.GraphQLVirtualHosts, cfg.Node.HTTPTimeouts); err != nil {
			utils.Fatalf("Failed to register the PUFFScoin service: %v", err)
		}
	}

	// Add the puffscoin Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
