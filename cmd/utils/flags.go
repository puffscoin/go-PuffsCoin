...
import (
	...
	"github.com/puffscoin/go-puffscoin/dashboard"
	...
	"github.com/puffscoin/go-puffscoin/p2p/netutil"
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "PUFFS", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the puffscoin service: %v", err)
	}
}
...
// RegisterEthStatsService configures the puffscoin Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Puffscoin
		ctx.Service(&ethServ)
		...
		Fatalf("Failed to register the puffscoin Stats service: %v", err)
	}
}
