...
import (
	...
	"github.com/puffscoin/go-puffscoin/ethstats"
	"github.com/puffscoin/go-puffscoin/les"
...
var (
	...
	netFlag     = flag.Uint64("network", 420, "Network ID to use for the PUFFScoin protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of puffs to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for puffscoin and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // puffscoin address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an puffscoin light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // puffscoin protocol stack
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the puffscoin light client protocol
...
// close terminates the puffscoin connection and tears down the faucet.
...
// apiHandler handles requests for puffs grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the puffscoin address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/puffscoin/go-puffscoin/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and puffscoin address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the puffscoin address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No puffscoin address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and puffscoin address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain puffscoin address,
